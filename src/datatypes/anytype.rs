...
// much of this code is adapted from the polars project
// I have made some changes to fit the needs of ylr
...
impl<'a> AnyValue<'a> {
    ...
    #[inline]
    pub fn try_extract<T: NumCast>(&self) -> Result<T> {
        self.extract()
            .ok_or_else(|| panic!("could not extract number from AnyValue of dtype: '{:?}'",))
    }
