...
use std::fs::File;
// import Arc to create a reference to the schema
use calm_io::stdout;
...
//#1A1D21,#121016,#FEC730,#1A1D21,#483F1E,#C6C0A8,#3F6AD3,#DB434B,#1A1D21,#C6C0A8

fn main() {
    // Open the CSV file
    let path = "data/titanic.csv".to_owned();
    ...
    // use arrow_csv to read the file with infered schema
    ...
    // for each column in the record batch match the datatype and return a formatted string like `tv` from `tidy-viewer` by alexhallam
    let record_batch_schema_clone = record_batch.schema().clone();

    // convert all arrays of anytype to Vec<Vec<String>>. A simple downcast_ref does not work here. It fails for Int64Array, Float64Array, etc.

    let record_batch_vec_vec_string: Vec<Vec<String>> = record_batch
        .columns()
        .iter()
        .map(|array| {
    ...
    if debug_mode {
        println!("{:?}", "record_batch_vec_vec_string");
        println!("{:?}", record_batch_vec_vec_string.clone());
        println!("{:?}", "Formatted: Vector of Elements vf_mut");
        println!("{:?}", vf_mut.clone());
        println!("{:?}", "Formatted: Vector of Elements vp");
        println!("{:?}", vp.clone());
    ...
    if !is_no_dimensions {
        ...
        let _ = match stdoutln!("{} {} {} {}", meta_text, rows_in_file, div, cols) {
            Ok(_) => Ok(()),
            Err(e) => match e.kind() {
                std::io::ErrorKind::BrokenPipe => Ok(()),
                _ => Err(e),
    ...
    // title
    if is_na(title_option.clone().to_string()) {
        let _ = match stdout!("{: >6}  ", "") {
            Ok(_) => Ok(()),
            Err(e) => match e.kind() {
                std::io::ErrorKind::BrokenPipe => Ok(()),
